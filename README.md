...
Универсальный инструментарий для AI-агентов, которые работают с **любыми прикладными конфигурациями 1С:Предприятие**, BSL и расширениями. Он не содержит привязки к УНФ или другой типовой конфигурации: выгружает только указанные пользователем метаданные, изолирует работу над расширениями от основной базы, проверяет результат пакетного Конфигуратора и удаляет созданные им временные данные.
...
Готовый универсальный сценарий находится в [`examples/safe-object-inspection`](examples/safe-object-inspection/README.md). Все параметры доступны через `py scripts/onec.py <команда> --help`.
...
| Конфигурации | Любые прикладные конфигурации | Нет жёстко заданных объектов; интеграционно проверено на УНФ |

Выборочная выгрузка требует платформу с поддержкой `-listFile`. Имена объектов всегда передаются явно, поэтому CLI одинаково работает с типовыми, отраслевыми и самописными конфигурациями. При создании расширения он сам определяет основной язык конфигурации и не предполагает, что это русский. УНФ используется только как документированная среда интеграционного теста, а не как ограничение инструментария.
...
Если инструментарий оказался полезен и сэкономил вам время, вы можете [поддержать его развитие через CloudTips](https://pay.cloudtips.ru/p/b5302504). Поддержка помогает проверять новые версии платформы и развивать безопасные сценарии работы с конфигурациями и расширениями 1С.
